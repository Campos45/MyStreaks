...
object DateHelper {
    ...
    fun isSameWeek(date1: Long, date2: Long): Boolean {
        ...
        val c1 = getCalendar(date1)
        val c2 = getCalendar(date2)
        return c1.get(Calendar.YEAR) == c2.get(Calendar.YEAR) && c1.get(Calendar.WEEK_OF_YEAR) == c2.get(
    ...
    fun isStreakBroken(frequency: String, lastInc: Long, now: Long): Boolean {
        ...
        return when (frequency) {
            "Diário" -> {
                ...
                cLast.set(Calendar.HOUR_OF_DAY, 0); cLast.set(Calendar.MINUTE, 0); cLast.set(
                    Calendar.SECOND, 0); cLast.set(Calendar.MILLISECOND, 0)
                cNow.set(Calendar.HOUR_OF_DAY, 0); cNow.set(Calendar.MINUTE, 0); cNow.set(Calendar.SECOND, 0); cNow.set(
